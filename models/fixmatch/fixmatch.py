...
class FixMatch:
    def __init__(self, args, net_builder, num_classes, ema_m, T, p_cutoff, lambda_u, \
        ...
        if args.pretrained_from == 'scratch':
            self.train_model = models.resnet50(pretrained=False, num_classes=10)
            self.eval_model = models.resnet50(pretrained=False, num_classes=10)
            # self.train_model.fc = nn.Linear(self.train_model.fc.in_features, 10)
            # self.eval_model.fc = nn.Linear(self.eval_model.fc.in_features, 10)
        elif args.pretrained_from == 'ImageNet_supervised':
            self.train_model = models.resnet50(pretrained=True)
            self.eval_model = models.resnet50(pretrained=True)
            self.train_model.fc = nn.Linear(self.train_model.fc.in_features, 10)
            self.eval_model.fc = nn.Linear(self.eval_model.fc.in_features, 10)
        elif args.pretrained_from == 'ImageNet_SimCLR' or args.pretrained_from == 'CIFAR_SimCLR' or args.pretrained_from== 'MLCC_SimCLR':
            self.train_model = models.resnet50(pretrained=False, num_classes=10)
            self.eval_model = models.resnet50(pretrained=False, num_classes=10)
            checkpoint_dir = os.path.join(args.pretrained_model_dir, args.dataset)
                ...
                if k.startswith('backbone.'):
                    if k.startswith('backbone') and not k.startswith('backbone.fc'):
                        ...
                        if k=='backbone.conv1.weight':
                            ...
                            print(state_dict[k])
                del state_dict[k]
            log = self.train_model.load_state_dict(state_dict, strict=False)
